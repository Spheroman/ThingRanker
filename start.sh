...
if ! [ -x "$(command -v docker compose)" ];
  then
    ...
    $sh_c apt-get update
    $sh_c apt-get install -y docker-ce docker-ce-cli containerd.io docker-buildx-plugin docker-compose-plugin git
fi

if ! [ -e ./ThingRanker/compose.yaml ]
  then
  $sh_c git fetch "https://github.com/Spheroman/ThingRanker.git"
  $sh_c cd ThingRanker
  $sh_c git checkout docker
  else
    $sh_c cd ThingRanker
fi
